...
export default function OutgoingPaymentsPage() {
  ...
  const changeBpType = (bpType) => {
    ...
    setSuccessMessage("");
    setInvoices([]);
    setPaymentOnAccount(["Account", "TDS", "PLA"].includes(bpType));
    ...
    setHeader((current) => ({
      ...
      contactPerson: "",
      blanketAgreement: "",
      controlAccount: ["Account", "TDS", "PLA"].includes(bpType) ? current.controlAccount : "",
    }));
    setJournalRemarks("");
  };
  ...
  const handleOutgoingPaymentSelect = (row) => {
    setLoadError("");
    setSuccessMessage("");
    ...
    setJournalRemarks(row.journalRemarks || `Outgoing Payments - ${row.businessPartnerCode || row.code}`);
    setPaymentOnAccount(Number(row.paymentOnAccountAmount || 0) > 0);
    setPaymentOnAccountAmount(money(row.paymentOnAccountAmount));
    setSuccessMessage(`Outgoing payment ${row.documentNo || row.code} loaded.`);
  ...
  const handleOk = async () => {
    ...
    if (!header.businessPartnerCode) {
      setLoadError("Select a business partner before adding the outgoing payment.");
  ...
  const isAccount = ["Account", "TDS", "PLA"].includes(header.bpType);
  ...
  return (
    <div className="po-page ip-payments-page">
      <div className="po-toolbar">
        <span className="po-toolbar__title">Outgoing Payments</span>
          ...
          <div className="sap-header-left">
            <div className="sap-bp-row">
              <div>
                <FieldRow label="Code">
                  <SapLookupField
                    ...
                    fetchOptions={(query) =>
                      isAccount
                        ? searchOutgoingPaymentControlAccounts(query)
                        : searchOutgoingPaymentBusinessPartners(query, header.bpType)
                    }
              ...
              <div className="sap-radio-stack">
                {["Vendor", "Customer", "Account", "TDS", "PLA"].map((type) => (
                  <label key={type}>
                    <input
                      type="radio"
                      name="op-bp-type"
