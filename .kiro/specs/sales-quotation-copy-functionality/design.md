# Design Document: Sales Quotation Copy From/Copy To Functionality

## Overview

This design implements SAP Business One-compatible Sales Quotation page with Copy From and Copy To functionality. The Sales Quotation module enables users to create quotations for customers by copying from source documents (AR Invoice, Delivery, Sales Order) with automatic base reference tracking. Copy To functionality allows propagating quotation data to target documents (AR Invoice, Delivery) for order fulfillment. The implementation maintains SAP B1 data integrity through BaseEntry, BaseType, and BaseLine fields.

## Architecture

```mermaid
graph TD
    A[Sales Quotation Page] -->|Copy From| B[Copy From Modal]
    ...
    A -->|Manage Tabs| J[Logistics/Accounting/Tax/Attachments]
```

## Components and Interfaces

### Component 1: SalesQuotation (Main Page)

**Purpose**: Main page component for creating and editing sales quotations with full CRUD operations.

**Interface**:
```javascript
interface SalesQuotationPage {
...
interface QuotationData {
  ...
  electronicDocuments: ElectronicDocumentData
}
```

**Responsibilities**:
- Display quotation form with all tabs
- Manage form state and validation
- Handle CRUD operations
- Coordinate Copy From/Copy To modals
- Calculate totals and taxes
- Manage batch allocations

### Component 2: CopyFromModal

**Purpose**: Display available source documents and allow user selection to populate quotation lines.
...
### Component 3: CopyToModal

**Purpose**: Display target documents and enable copying quotation data for order fulfillment.
...
- Pass quotation data to backend for creation
- Handle success/error responses

### Component 4: ContentsTab

**Purpose**: Manage quotation line items with item selection and batch allocation.

**Interface**:
```javascript
interface ContentsTab {
  lines: LineData[]
  onAddLine(line: LineData): void
  onRemoveLine(lineNum: number): void
  onUpdateLine(lineNum: number, line: LineData): void
  onBatchAllocation(lineNum: number): void
}
...
**Responsibilities**:
- Display item matrix with all line fields
- Allow manual item addition via ItemSelectionModal
- Support line editing and deletion
- Trigger batch allocation for batch-managed items
- Recalculate line totals and taxes

### Component 5: LogisticsTab

**Purpose**: Manage shipping and delivery details.

**Interface**:
```javascript
interface LogisticsTab {
  data: LogisticsData
...
**Responsibilities**:
- Display shipping type selection
- Manage delivery dates
- Handle address selection
- Display freight charges

### Component 6: AccountingTab

**Purpose**: Manage payment terms and accounting details.

**Interface**:
```javascript
interface AccountingTab {
  data: AccountingData
  onUpdate(data: AccountingData): void
}
...
**Responsibilities**:
- Display payment terms selection
- Manage payment method
- Handle owner and purchaser assignment
- Display branch and warehouse selection

### Component 7: TaxTab

**Purpose**: Manage tax calculations and GST details.
...
interface TaxData {
  taxCode: string
  gstType: 'INTERSTATE' | 'INTRASTATE' | 'OTHER'
  igst: number
  cgst: number
  sgst: number
  totalTax: number
...
**Responsibilities**:
- Display tax code selection
- Calculate GST components
- Display tax breakdown

### Component 8: ElectronicDocumentsTab

**Purpose**: Manage e-invoice and e-way bill support.

**Interface**:
```javascript
interface ElectronicDocumentsTab {
  data: ElectronicDocumentData
  onUpdate(data: ElectronicDocumentData): void
}

interface ElectronicDocumentData {
  eInvoiceStatus: 'pending' | 'generated' | 'failed'
  eInvoiceNumber: string
  eWayBillStatus: 'pending' | 'generated' | 'failed'
  eWayBillNumber: string
}
```

**Responsibilities**:
- Display e-invoice status and number
- Display e-way bill status and number
- Handle e-document generation

### Component 9: AttachmentsTab

**Purpose**: Manage document attachments.
...
### Component 10: ItemSelectionModal

**Purpose**: Allow manual item selection for adding to quotation.
...
**Responsibilities**:
- Display searchable item list
- Allow item selection
- Pass selected item to parent component

### Component 11: BatchAllocationModal

**Purpose**: Manage batch allocation for batch-managed items.

**Interface**:
```javascript
interface BatchAllocationModal {
  isOpen: boolean
  itemCode: string
  warehouse: string
  quantity: number
  onSave(batches: BatchAllocation[]): void
  onClose(): void
}
...
**Responsibilities**:
- Display available batches for item
- Allow batch selection and quantity allocation
- Validate total batch quantity matches line quantity
- Pass batch allocations to parent component
...
BEGIN
  ...
  // Step 3: Transform lines to quotation format
...
```javascript
function populateItemMatrix(quotationLines, sourceLines)
```

**Preconditions**:
- `quotationLines` is array of line objects
...
const quotationLines = sourceLines.map(line => ({
...
```javascript
// Step 1: User saves Sales Quotation
const quotationData = {
  docEntry: 456,
  docNum: 'QT-001',
  header: {
...
showNotification('Invoice created from quotation: INV-003');
```
...
```javascript
// All copied lines must belong to same customer as quotation

∀ line ∈ quotationLines:
  IF line.baseEntry ≠ null THEN
    getSourceDocumentCustomer(line.baseEntry, line.baseType) = 
    quotationHeader.cardCode
```
...
∀ sourceDoc ∈ selectedSourceDocuments:
  ...
  sourceDoc.cardCode = quotationHeader.cardCode
```
...
**Condition**: Source document belongs to different customer than quotation
**Response**: Display error: "Source document customer does not match quotation customer."
...
1. Validate copy to eligibility - verify quotation is saved
...
2. End-to-end Copy To: Save quotation → Select target → Create reverse transaction → Verify creation
...
**Authorization**:
- Verify user has permission to create/edit Sales Quotations
